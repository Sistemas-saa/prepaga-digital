...
function flattenSaleData(sale: any): Record<string, string> {
  ...
  const formatDateLongLocal = (dateValue: string | null | undefined) => {
    if (!dateValue) return "";
    return new Date(dateValue.includes("T") ? dateValue : `${dateValue}T00:00:00`).toLocaleDateString("es-PY", {
      day: "numeric",
      month: "long",
      year: "numeric",
    });
  ...
  if (sale) {
    ...
    result["contract_start_date"] = formatDateLongLocal(sale.contract_start_date);
